...
public class CreativeTabs {
	...
	public static final CreativeTabName[] CREATIVE_TABS = { new CreativeTabName("more_generators_items", ModItems.BISMUTH),
			new CreativeTabName("more_generators_blocks", ModBlocks.BISMUTH_BLOCK) };
	...
	public static void register(IEventBus eventBus) {
		for (Map.Entry<CreativeTabName, List<ItemLike>> entry : ITEMS.entrySet()) {
			...
			CreativeModeTab.Builder tabBuilder = CreativeModeTab.builder().icon(icon)
					.title(Component.translatable("creativetab." + MoreGenerators.MOD_ID + "." + tabName.name()))
					.displayItems((itemDisplayParams, output) -> items.forEach(output::accept));
