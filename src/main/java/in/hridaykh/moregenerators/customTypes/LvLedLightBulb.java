...
public class LvLedLightBulb extends LightBulb {
	...
	public LvLedLightBulb(Item.Properties settings) {
		...
		this.modelSupplier = () -> state -> switch (state) {
			case OFF -> ModPartialModels.LV_MODEL_OFF;
			case LOW_POWER, ON -> ModPartialModels.LV_MODEL_ON;
			case BROKEN -> ModPartialModels.LV_MODEL_BROKEN;
			case LIGHT -> ModPartialModels.LV_MODEL_LIGHT;
		};
		this.dyedModelSupplier = () -> state -> switch (state) {
			case OFF -> ModPartialModels.LV_DYED_MODEL_OFF;
			case LOW_POWER, ON -> ModPartialModels.LV_DYED_MODEL_ON;
			case BROKEN -> ModPartialModels.LV_DYED_MODEL_BROKEN;
			case LIGHT -> ModPartialModels.LV_DYED_MODEL_LIGHT;
			case BULB -> ModPartialModels.LV_DYED_MODEL_BULB;
		};
		applyRatedValues(
			RATED_POWER_WATTS,
			RATED_VOLTAGE_VOLTS,
	...
	// Modern 1.21.1 ResourceLocation construction
	private static ResourceLocation powerGridId(String path) {
		return ResourceLocation.fromNamespaceAndPath(MoreGenerators.MOD_ID, path);
	}
	...
	// Modernized 1.21.1 context signature for appendHoverText
	@Override
